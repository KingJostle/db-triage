...
| 4 | **Paste mode** — no direct access | none of the above | Hand the user *one* command; parse what comes back | Whatever they paste. List explicitly which checks were not in it. |
...
If `checks/registry.csv` and `reference/` sit next to this file, **use them instead of the
embedded pass below**: run `checks/postgres/fast-cluster.sql` once per cluster and
`checks/postgres/fast-database.sql` once per scanned database (at most five, largest
first), read priorities and thresholds from `checks/registry.csv`, and cite each finding's
`ref` anchor as well as its `docs_url`. `bin/db-triage --format markdown` does all of that
and renders the report.

If they are not present, run the embedded pass below, cite `docs_url` only, and emit
