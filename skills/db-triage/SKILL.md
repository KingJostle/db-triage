...
name: db-triage
description: >-
  Read-only, prioritized health check for an unfamiliar PostgreSQL or MySQL/MariaDB
  instance — sp_Blitz for Postgres. Use when asked for a database health check, triage,
  audit or review; "what's wrong with this database", "is this Postgres safe", "check my
  database before we launch", "sp_Blitz for postgres", "why is this database slow",
  "review this RDS instance". Produces a ranked punch list: what gets you fired, what to
  fix this week, what to plan, and what is merely worth knowing. Never writes.
metadata:
  version: "0.1.0"
---
...
A first responder for an unfamiliar database. It establishes the patient's condition and
orders the problems by how badly they will hurt. It never treats: every recommendation is
text for a human to run.

Three sentences of scope: point it at a PostgreSQL or MySQL instance cold, get back a
ranked punch list with the mechanism, the measured numbers and the next action for each
finding. It is not a tuning tool, not a monitor, not a corruption checker, and not a query
tuner — it reports signals and points at the tool that goes deeper. Everything it says
carries a confidence level, because the biggest way this class of tool fails is by
sounding sure.

---

## 1. The safety contract (normative — do not deviate)

**db-triage must be safe to point at a production primary during business hours.**

| Rule | How it is enforced |
|---|---|
| **Read-only, always.** Only `SELECT`, `SHOW`, read-only psql metacommands, and session-scoped `SET`. | The session opens with `SET default_transaction_read_only = on`, so a mistake fails rather than writes. |
| **Bounded.** No statement runs unbounded; no catalog read queues behind DDL. | `statement_timeout = 10s` (fast) / `60s` (deep), `lock_timeout = 2s`, `idle_in_transaction_session_timeout = 60s`. |
| **Identifiable.** The DBA can see and cancel the run. | `application_name = 'db-triage/0.1.0'`, visible in `pg_stat_activity`. |
| **Never mutates state, including statistics.** | Never run: `VACUUM`, `ANALYZE`, `CLUSTER`, `REINDEX`, `CHECKPOINT`, `pg_switch_wal()`, any `pg_stat_reset*()`, `pg_terminate_backend()`, `pg_cancel_backend()`, `pg_drop_replication_slot()`, `ALTER SYSTEM`, `CREATE EXTENSION`, `CREATE TEMP TABLE` (use CTEs), `SELECT … FOR UPDATE`, `pg_advisory_lock*`. MySQL: never `OPTIMIZE`, `ANALYZE TABLE`, `CHECK TABLE`, `REPAIR`, `FLUSH *`, `KILL`, `SET GLOBAL`, `SET PERSIST`, `START/STOP REPLICA`. |
| **No heavy scans.** No check reads user table *data*. | `count(*)` on user tables is forbidden. Bloat is estimated from `pg_stats` (catalog-only); `pgstattuple_approx()` is deep-pass only, per-relation, with a size cap. |
| **Never acts on the user's behalf.** | Every "next action" is text. If db-triage suggests `VACUUM FREEZE`, the human runs it. |
| **Never reads secrets.** | Password hashes are tested for NULL/format only and never echoed. `primary_conninfo` is emitted with the password redacted. Connection strings never appear in a report. |

**Cost tiers.** `0` = catalog/settings only, sub-millisecond, always run. `1` = cumulative
statistics views or per-relation catalog joins; milliseconds, seconds above 100k relations.
`2` = reads relation pages, log files or an extension call; **deep pass only, each one
individually confirmed by the user.**

State the promise in one line before running: *"Read-only, bounded, and identifiable as
db-triage in pg_stat_activity. Nothing is changed."*

---

## 2. Access ladder

Evaluate in order, use the first that works, and say which rung was used in the report.

| # | Rung | How to detect it | How to run | Limits |
|---|---|---|---|---|
| 1 | Shell + `psql`/`mysql` that can reach the DB | `which psql`; `PG*` / `DATABASE_URL` / `MYSQL_*` env; `~/.pgpass`, `~/.my.cnf`; a DSN the user pasted | `bin/db-triage` if this repo is present, else `psql -f checks/postgres/fast-cluster.sql`, else the embedded pass in §8 | Full catalog. OS checks only if the shell is on the DB host. |
| 2 | MCP database connector that executes SQL | a SQL-execution tool in the tool list | Send the session SETs first, then each check as one statement; evaluate against the thresholds inline | No metacommands; one database per connection; no OS or log checks; possibly no `SET` — then run cost 0/1 only and record `session-safety: not-applied` |
| 3 | A read replica or reporting copy | the user says so, or preflight shows `pg_is_in_recovery()` | as rung 1 or 2 | Primary-only checks skipped; usage counters are the replica's; emit `XX-META-005` |
| 4 | **Paste mode** — no direct access | none of the above | Hand the user the two-line block below; parse what comes back | Whatever they paste. List explicitly which checks were not in it. |
| 5 | The user pastes something else (a `pg_settings` dump, `SHOW VARIABLES`, an error log) | content sniffing | Evaluate every check whose inputs are present | Partial by construction; say so in the first line of the report |

**Paste mode, verbatim.** Save the SQL in §8 to a file and hand over exactly this:

```
psql "$YOUR_DSN" -X -q -f db-triage-preflight.sql --csv >  triage.csv 2>&1
psql "$YOUR_DSN" -X -q -f db-triage-fast.sql      --csv >> triage.csv 2>&1
# then paste triage.csv back here
```

The preflight line is not optional padding: without it the platform is unknown, so
`platform_skip` cannot be applied and a managed instance reports provider-owned settings
(`fsync`, `archive_mode`, `ssl`) as findings. A paste that omits it is still evaluated, and
the report says in its caveats that the platform adaptations did not run.

Never: guess or brute-force credentials; write a DSN into any file; connect as superuser
when a `pg_monitor` role is on offer.

**Ask at most three questions up front**, and only ones preflight cannot answer:
1. How do I reach it? (only if no DSN is discoverable)
2. Is this production, and is it the primary? (preflight usually knows)
3. The backup interview: what takes backups, when did one last succeed, when was a
   restore last tested? Record the answer so it is never asked twice.

---

## 3. Preflight

Run this first. Everything after it depends on the answers.
...
SELECT 'postgresql' AS engine,
       ...
       (SELECT min(stats_reset) FROM pg_stat_database) AS earliest_stats_reset,
       CASE WHEN EXISTS (SELECT 1 FROM pg_proc  WHERE proname='aurora_version')      THEN 'aurora'
            WHEN EXISTS (SELECT 1 FROM pg_roles WHERE rolname='rdsadmin')            THEN 'rds'
            WHEN EXISTS (SELECT 1 FROM pg_roles WHERE rolname IN ('cloudsqladmin','cloudsqlsuperuser')) THEN 'cloudsql'
            WHEN EXISTS (SELECT 1 FROM pg_roles WHERE rolname='azure_pg_admin')      THEN 'azure'
            ...
            ELSE 'self-managed' END AS platform;
SELECT datname, pg_database_size(oid) AS size_bytes, datallowconn, datistemplate
FROM pg_database ORDER BY pg_database_size(oid) DESC;
```

**Managed platforms** (RDS, Aurora, Cloud SQL, Azure Flexible, Supabase, Neon, Timescale,
AlloyDB, Heroku) withhold superuser: `pg_hba_file_rules`, log files and `pg_ls_waldir()`
are unavailable, and the provider owns backups. On those, skip `PG-BAK-001` (report
"confirm retention > 0 and PITR in the console" instead), skip the HBA checks, and expect
`shared_buffers`, checksums and `wal_level` to be set by the platform. Everything else —
wraparound, bloat, slots, locks, indexes, roles, workload — runs unchanged. Emit
`XX-META-006` listing what changed.

**MySQL preflight:** `SELECT @@version, @@version_comment, @@read_only, @@super_read_only,
@@log_bin, @@gtid_mode, @@performance_schema, @@innodb_force_recovery;` plus
`SHOW REPLICA STATUS` (8.0.22+) or `SHOW SLAVE STATUS` (MariaDB, older MySQL).

---

## 4. Session safety block

Send this **before every batch**, on every rung that allows `SET`.

```sql
SET application_name = 'db-triage/0.1.0';
SET default_transaction_read_only = on;
SET statement_timeout = '10s';        -- '60s' for the deep pass
SET lock_timeout = '2s';
...
SET SESSION max_execution_time = 10000;   -- MySQL 8; MariaDB: SET SESSION max_statement_time = 10
SET SESSION lock_wait_timeout = 2;
```

If the connector refuses `SET`, run cost 0 and 1 checks only and record
`session-safety: not-applied` in the run metadata. Do not proceed without saying so.

---

## 5. Priority bands

One integer, lower is worse, so a security finding, a vacuum finding and a backup finding
interleave correctly in one list. Sparse on purpose, so future checks slot in without
renumbering.

| P | Band | Meaning |
|---|---|---|
| **0** | Meta | The run itself is incomplete or its inputs are unreliable. Read before believing anything else. |
| **1** | You get fired | Data loss or a hard outage is happening or is hours-to-days away. Drop everything. |
...
| **20** | Known-dangerous | Needs a plan this month; usually a restart, an upgrade or a schema change. |
| **50** | Daily-briefing ceiling | Real problems worth fixing this week. |
| **100** | Tuning detail | Fix when convenient; measurable but not urgent. |
| **150** | Hygiene / low-confidence | Worth a look; may well be intentional. |
| **200** | Non-default config | Inventory of every setting that differs from the default. Not problems. |
| **230** | Security review | Lists a reviewer signs off on: superusers, privileged attributes, file access. |
| **240** | Workload profile | Top queries, wait-event snapshot. Raw material for the next step. |
| **250** | Environment inventory | What this thing is: version, platform, role, sizes, extensions, topology. |
| **254** / **255** | Run metadata / credits | |

**Documented ceilings.** `≤10` pager band — wake someone up. `≤50` daily-job band — the
default cut-off for a scheduled unattended run. `≤150` weekly review. All — the full
consulting deliverable.

P1 is reserved. A replica six minutes behind is bad; it is not "the database refuses
writes tomorrow". Crying wolf at P1 costs the whole tool its credibility.

**Categories** (used in check IDs and in the rollup): `META` run integrity · `WRAP`
wraparound & freeze (PG) · `UNDO` undo & purge (MySQL) · `VAC` autovacuum & bloat ·
`BAK` backup & recovery · `DUR` durability · `CORR` corruption signals · `REPL`
replication & HA · `WAL` checkpoints & write-ahead log · `MEM` memory & caching ·
`CONN` connections & pooling · `LOCK` locking & long transactions · `SEC` security ·
`IDX` indexes · `SCHEMA` schema design · `QRY` queries & workload visibility ·
`CAP` capacity & growth · `REL` reliability & operations · `CFG` non-default config ·
...
Check IDs are `<ENGINE>-<CAT>-<NNN>`, stable forever, never reused, never renumbered.
Magnitude tiers of one condition get separate IDs (`PG-WRAP-001` P1 at 1.5 B,
`PG-WRAP-002` P10 at 1.0 B) so suppressing the mild tier never hides the severe one.

| Field | Rule |
|---|---|
| `check_id`, `priority`, `category`, `title` | From the catalog. Copy the priority into the finding so the report is self-describing. |
| `scope` | `cluster`, `database`, `schema`, `relation`, `index`, `role`, `setting`, `session`, `slot`, `replica`, `query`, `host`, `interview`. |
| `object` | The most specific thing the finding is about, fully qualified: `db.schema.relation`, `pid:NNN`, a slot name, `queryid:NNN`, or null for cluster-wide. |
| `details` | **Built from the actual values found, with units and the threshold that was crossed.** If two different servers would produce identical `details` for a check that measures something, the check is wrong. |
| `evidence` | The raw numbers used to decide, flat, snake_case, units in the key name (`_bytes`, `_seconds`, `_pct`). This is what makes the report auditable and diffable. |
| `confidence` | `high` = a catalog fact. `medium` = counter-based since reset, or a heuristic with known blind spots. `low` = an estimate, a regex over query text, or a missing input. |
| `next_action` | One imperative sentence. May name another check to consult first. Never a command db-triage runs. |
| `fix_effort` / `fix_risk` | S/M/L · low/med/high. |

Three emission shapes: one finding per offending object; one summary finding with a count
and a top-N list when more than 20 objects would fire; one finding when a cluster-level
condition is true.

**Language rules.** Lead with the mechanism, not the metric — "commits are hanging" before
"`sync_state` has no `sync` row". Every number carries a unit and, if it is a rate, the
window it was measured over. Use *reported* (a catalog says so), *estimated* (a model says
so), *since reset / over N days* (a counter says so), *at snapshot time* (a sample says
so), *no evidence of* (absence). Never "corrupt", "broken" or "unused" without the
qualifier that applies. No emoji, no exclamation marks — the priority integer is the
emphasis.
...
```markdown
# db-triage — <target> — <YYYY-MM-DD HH:MM UTC>

**Verdict:** <one line>. e.g. "2 findings need action today; backups unverified.
PRIMARY · PostgreSQL 15.6 · self-managed · fast mode · 118 checks run, 6 skipped ·
counters cover 41 days."

## 0. Read this first (META)      ← only when P0 rows exist
## 1. Fix first                   ← at most 5, at most 2 per category, ordered
### 1. [P1] PG-BAK-002 — archive_command is failing — cluster
**What:** the dynamic details, with the measured numbers.
**Do:** ordered steps, dependencies explicit ("blocked until step 1").
**Validate:** the query or observation that proves it worked.
**If it goes wrong:** the failure mode and how to reverse it.
**Effort / risk:** S / low · Ref: … · Docs: …
## 2. Rollup                      ← category × priority-band matrix, plus a one-liner:
   Fired: 42 · Skipped: 9 · Suppressed: 3 · Counters cover: 41 days · Access: rung 1
## 3. Ranked punch list           ← every non-suppressed finding ≤ P150, one table
   | P | ID | Category | Finding | Object | Details | Next action | Effort/Risk | Conf |
## 4. Configuration inventory (P200)   ← collapsed <details>
## 5. Security review (P230)           ← collapsed
## 6. Workload profile (P240)
## 7. Environment (P250)
## 8. Caveats                     ← auto-built; see below
## 9. What to run next            ← deep pass, query tuning, index review, backup test
## Appendix A. Checks skipped, with reasons
## Appendix B. Suppressed findings
## Appendix C. Run metadata
```

**Rules for "Fix first".** Lowest priority number wins; at most five items; at most two
per category; on ties prefer the lower `fix_effort`. Every P≤10 item gets **Validate** and
**If it goes wrong**. A low-confidence finding at P≤10 is titled `[P1, unverified]` — never
silently dropped and never silently asserted.

**The Caveats section is mandatory** and is built from: every META row; the statistics
window; every `low`-confidence finding at P≤50 with the reason it is low; any check that
needed RAM, disk or an interview answer that was missing; and the platform notes.

**Known ways this class of tool misleads** — say these when they apply:

- **Bloat is an estimate**, wrong on TOAST-heavy or freshly-analyzed tables. Never quote a
  percentage without the size basis; recommend `pgstattuple_approx` before any rewrite.
- **Counters are "since reset", not "per day".** A reset, a restart or three years of
  uptime all change what every rate means. Windows under 24 h force `confidence: low`.
- **Usage stats are per instance.** An index unused on the primary may serve the replica's
  reports. Say "on this instance" and "verify on every replica before dropping".
- **Snapshots lie about "right now".** One `pg_stat_activity` sample misses a lock storm
  and catches one-offs. Say "at snapshot time".
- **Absence of evidence is not evidence of absence.** No monitoring role visible ≠ no
  monitoring. Phrase as "no evidence of", and let the interview answer stop it firing.
- **Thresholds are opinions.** 90 % of `max_connections` is fine behind a pooler;
  `random_page_cost = 4` is right on spinning disks.
- **Dangerous-looking defaults that are fine in context** — `fsync=off` on a throwaway CI
  database. Do not auto-suppress; invite `environment: dev` in the config instead.

---

## 8. Suppression and baseline file

Lives with the user (`./.db-triage.yml`, `~/.config/db-triage/<target>.yml`), never in the
tool, so it survives upgrades. Honour it; never silently drop a finding.

```yaml
target: prod-pg-1
baseline:
  ram_gb: 64
  storage: nvme            # hdd|ssd|nvme|cloud|unknown
  disk_total_gb: 2000
  expected_superusers: [postgres, dba_admin]
  supervisor: systemd
interview:
  last_backup_success: 2026-09-01
  last_restore_test: 2026-06-14
thresholds:
  PG-REPL-006: {lag_seconds: 900}
skip:
  - check: PG-IDX-002
    object: "reporting.public.events_legacy_idx"
    reason: "used only by the quarterly replica job"    # reason is mandatory
    until: 2027-01-01                                   # expired skips are ignored AND reported
```

A suppressed finding is still emitted in JSON with `suppressed: true`; the Markdown report
says "n findings suppressed (see appendix)". An override naming a threshold key that does
not exist aborts the run — a typo must never silently disable a check.

---

## 9. If the full repository is present

**Find the catalog first.** `checks/registry.csv` is the file to look for, and it sits at
the repository root. Depending on how db-triage was installed that root is either the
directory holding this file (a clone into `~/.claude/skills/db-triage`) or two levels above
it (`../../` — the plugin layout, where this file is `skills/db-triage/SKILL.md`). Check
both, then `~/Projects/SQLSkill`, then any other `db-triage` checkout in reach.

**When the catalog is found, use it instead of the embedded pass below**: run
`<root>/checks/postgres/fast-cluster.sql` once per cluster and
`<root>/checks/postgres/fast-database.sql` once per scanned database (at most five,
largest first), read priorities and thresholds from `<root>/checks/registry.csv`, and cite
each finding's `ref` anchor as well as its `docs_url`. `<root>/bin/db-triage --format
markdown` does all of that and renders the report. It is 376 checks against 41 — always
prefer it.

If no clone is reachable, run the embedded pass below, cite `docs_url` only, and emit
`XX-META-001` with reason `embedded-subset` listing what a full run would have added: the
per-object index, bloat, sequence, workload and inventory checks, and every deep-pass check.

---

## 10. Embedded fast pass (PostgreSQL)

Covers every check at priority 10 or below plus the inventory rows needed to read the rest.
Runs on PostgreSQL 11–17. Each statement returns `check_id, scope, object, details,
evidence_json, confidence`; the `-- CHECK <id> P<n> <title>` markers carry the priority and
title to use in the report. Run the §4 session block first.

<!-- BEGIN EMBEDDED FAST PASS — generated by bin/build.py; do not hand-edit -->
...
<!-- END EMBEDDED FAST PASS -->

## 11. What this deliberately does not do

Guess or test passwords. Estimate restore time (there is no backup history in Postgres —
the interview is the honest substitute). Analyse deadlock graphs. Run sampling loops beyond
the three wait samples in deep mode. Recommend a specific index definition: it points at
the table and the statements and stops there.

---

*db-triage is modelled on the methodology of Brent Ozar's SQL Server First Responder Kit
(`sp_Blitz`) — one integer priority, stable check IDs, dynamic details, a documented
daily-job ceiling, suppression that lives outside the tool. No code was copied; every
check is derived from how PostgreSQL and MySQL actually fail. Not affiliated with or
endorsed by Brent Ozar Unlimited.*
